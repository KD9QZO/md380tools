...
//
// This means they should never be reordered, or the settings will get tangled up.
typedef struct addl_config {
	...
	uint8_t cw_volume;				// output 'volume' (PWM duty cycle) for CW messages. 0..100 [%] for fixed volume, or BEEP_VOLUME_AUTO to control via pot:
#define BEEP_VOLUME_AUTO 255
	...
	uint16_t edit_bg_color;			// fg colour of a cell in "edit mode" (or, single char for the CURSOR)
									// The above 16-bit colours use the display's native 'BGR565'-format, as used for
									// the 'alternative' menu and the alternative LCD driver:
...
#endif	/* !MD380TOOLS_ADDL_CONFIG_H_INCLUDED */
