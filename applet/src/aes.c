...
/*
 * These Motorola Basic Privacy keys are sampled manually from silent frames in the air; as such, they are imperfect and
 * likely contain flipped bits. A better method would be to extract the complete sequence from either Motorola firmware
 * or automatically fetch them from the first frame of a transmission.
 */
...
/* This hook intercepts calls to an initial AES check at startup that may (or may not) be related to the ALPU-MP chip.
 *
 * Is not related to the ALPU-MP chip (20160720ae) ALPU-MP "lib" has his own ?-Crypto
*/
void aes_startup_check_hook() {
	...
	/*
	 * aes_startup_check() will set the byte at 0x2001d39b to 0x42, which is then checked repeatedly elsewhere in the
	 * code, causing mysterious things to fail. If we find that this check has failed, let's force it back the other way
...
/* This hook intercepts calls to aes_loadkey(), so that AES keys can be printed to the dmesg log. */
