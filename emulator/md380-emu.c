...
void mapimage() {
	...
	tcram = mmap((void*)0x10000000, (size_t)0x20000, (PROT_READ | PROT_WRITE) /* <--- protections */, MAP_PRIVATE /* <--- flags */, fdtcram /* <--- file */, 0 /* <--- offset */);

	if (firmware == (void*)-1 || sram == (void*)-1 || tcram == (void*)-1) {
		printf("mmap() error %i.\n", errno);
		exit(1);
	}
}

/* We declare the function weak for places where it's unknown, as loadfirmwareversion() can't be found by symgrate. */
...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "edVvo:i:")) != -1) {
		switch (opt) {
			/* For any flag that sets the mode, we simply set the verb char to that mode. */
			case 'V'://Version
			case 'd'://Decode AMBE
			case 'e'://Encode AMBE
			case 'h'://usage
